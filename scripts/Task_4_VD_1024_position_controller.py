...
class Control:
	...
	def polar(self, y, x):
		r = self.meters_to_long(abs(complex(x,y)))
		theta = math.degrees(math.atan(x/y)) if y else 90 if x else 0
		return -r if y < 0 else r, -theta

	def shifted_coordinates(self, x, y):
		x = self.lat_to_x(x)
		y = self.lon_to_x(y)
		return self.metres_to_lat(x*math.cos(self.theta_pos) + y*math.sin(self.theta_pos)), self.meters_to_long(-x*math.sin(self.theta_pos) + y*math.cos(self.theta_pos))

	def theta_pos_callback(self, msg):
		self.theta_pos = msg.data

	def yaw_err_callback(self, msg):
		self.yaw_err = msg.data
	...
	def cmd(self):
		...
		if True:
			...
			for i in range(3):
				...
				# while using polar coordinate system
				if self.using_polar:
					self.error[0], self.error[1] = self.shifted_coordinates(self.error[0], self.error[1]) #if abs(self.yaw_err) < 1.2 else (0,0)
					print(math.degrees(self.theta_pos), format(self.lat_to_x(self.error[0]), "f"), format(self.lon_to_x(self.error[1]), "f"))
